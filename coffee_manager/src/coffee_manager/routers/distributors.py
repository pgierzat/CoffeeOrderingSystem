...
def _replace_prices(
    db: Session, dist: Distributor, daily_prices: list[DailyPrice]
) -> None:
    for p in dist.daily_prices:
        db.delete(p)
    db.flush()
    for price in daily_prices:
        dp = DistributorDailyPrice(
...
def _replace_delivery_params(
    db: Session, dist: Distributor, delivery_params: list[DeliveryParams]
) -> None:
    for p in dist.delivery_params:
        db.delete(p)
    db.flush()
...
@router.post(
    "", response_model=DistributorResponse, status_code=status.HTTP_201_CREATED
)
...
def _apply_update(
    db: Session, dist: Distributor, body: DistributorUpdateRequest
) -> None:
